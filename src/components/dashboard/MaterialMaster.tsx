...
const MaterialMaster: React.FC = () => {
  ...
  const forecastMap = useMemo(() => {
    ...
    const result: Record<
      string,
      { outlook: "Stable" | "Watch" | "Reorder Soon"; daysCover: number | null }
    > = {};

    for (const material of materials) {
      const relevantOut = movements.filter((mv) => {
        const movementDate = mv.createdAt ? new Date(mv.createdAt) : null;
        return (
          mv.type === "OUT" &&
          normalize(mv.materialName) === normalize(material.name) &&
          movementDate &&
          movementDate >= last30Days
        );
      });

      const totalOut30 = relevantOut.reduce(
        (sum, mv) => sum + Number(mv.quantity || 0),
        0
      );

      const avgDailyOut = totalOut30 / 30;
      const currentStock = Number(material.currentStock || 0);
      const minStock = Number(material.minStock || 0);

      let daysCover: number | null = null;
      if (avgDailyOut > 0) {
        daysCover = currentStock / avgDailyOut;
      }

      let outlook: "Stable" | "Watch" | "Reorder Soon" = "Stable";

      if (currentStock <= minStock) {
        outlook = "Reorder Soon";
      ...
      result[material._id] = { outlook, daysCover };
    }
  ...
  const getForecastClasses = (outlook: "Stable" | "Watch" | "Reorder Soon") => {
    if (outlook === "Reorder Soon") {
        ...
        <table className="w-full">
          <thead>
            <tr className={`border-b ${isDark ? "border-gray-700" : "border-gray-200"}`}>
              ...
              <th className="px-2 py-3 text-sm font-semibold text-left">Forecast</th>
          ...
          <tbody>
            {materials.map((material) => {
              const forecast = forecastMap[material._id];
              const outlook = forecast?.outlook ?? "Stable";
              const daysCoverText =
                forecast?.daysCover == null
                  ? "No usage trend yet"
                  : `${Math.max(1, Math.round(forecast.daysCover))}d cover`;
                ...
                >
                  ...
                  <td className="px-2 py-3">
                    <div className="flex flex-col gap-1">
                      <span
                        className={`inline-flex w-fit px-2 py-1 text-xs font-medium rounded-full ${getForecastClasses(
                          outlook
                        )}`}
                      >
                        {outlook}
